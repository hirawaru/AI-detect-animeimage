...
#### 3.4.3 Kết quả huấn luyện (20 Epochs)

Dưới đây là kết quả huấn luyện sau khi hoàn thành 20 epochs. Mô hình đã đạt được độ chính xác rất cao và có khả năng phân loại ổn định:

| Epoch | Train Loss | Train Acc | Val Loss | Val Acc |
|-------|-----------|-----------|----------|---------|
| 1 | 0.4792 | 78.34% | 0.4837 | 83.23% |
| 5 | 0.3283 | 85.71% | 0.2913 | 87.33% |
| 10 | 0.2085 | 91.87% | 0.2337 | 91.80% |
| 15 | 0.0970 | 96.50% | 0.6110 | 91.43% |
| 20 | 0.0508 | 98.30% | 0.5547 | **91.89%** |

**Nhận xét:**
- **Sự hội tụ:** Mô hình hội tụ rất nhanh nhờ sử dụng kỹ thuật Transfer Learning từ EfficientNet-B3. Chỉ sau 10 epochs, Accuracy đã vượt mức 90%.
- **Độ chính xác:** Kết quả cao nhất trên tập validation đạt **91.89%** tại epoch 20.
- **Overfitting:** Có dấu hiệu Overfitting nhẹ ở các epoch cuối (Train Acc > 98% trong khi Val Acc đi ngang ở mức 91-92%). Tuy nhiên, nhờ cơ chế Early Stopping và Save Best Model, chúng ta luôn giữ được phiên bản mô hình có khả năng tổng quát hóa tốt nhất (tại epoch 10 với Val Loss thấp nhất là 0.2337).

**Kết quả đánh giá trên tập Test:**
Sau khi huấn luyện xong, mô hình được đánh giá độc lập trên tập dữ liệu Test (chưa từng xuất hiện trong quá trình train/val):

- **Accuracy:** 91.89%
- **F1-Score:** 91.18%
- **ROC-AUC:** 97.37%

*Ảnh minh họa Confusion Matrix (results_20epochs/confusion_matrix.png):* Cho thấy mô hình nhận diện tốt cả hai lớp Natural và Synthetic, với tỉ lệ nhầm lẫn thấp.

*Ảnh minh họa ROC Curve (results_20epochs/roc_curve.png):* Đường cong tiệm cận góc trên bên trái với AUC = 0.97, khẳng định khả năng phân biệt cực kỳ mạnh mẽ của mô hình.
...
**Hạn chế về mô hình:**
- Mô hình được huấn luyện trên môi trường CPU nên thời gian training khá lâu, hạn chế việc thử nghiệm nhiều kiến trúc phức tạp hơn.
...
1. **Transfer Learning là chìa khóa:** Với dataset nhỏ (~6,543 ảnh), việc sử dụng pretrained backbone giúp mô hình hội tụ nhanh hơn nhiều so với training từ đầu. Chỉ sau 20 epochs đã đạt 91.89% accuracy.
